...
class UtmconverterAPIClientError(Exception):
...
class ValidationError(UtmconverterAPIClientError):
...
class UtmconverterAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"direction": {"type": "string", "required": False, "default": "to_utm"}, "latitude": {"type": "number", "required": True, "min": -90, "max": 90}, "longitude": {"type": "number", "required": True, "min": -180, "max": 180}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the UtmconverterAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises UtmconverterAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/utmconverter'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise UtmconverterAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[UtmconverterAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns UTMCoordinateConverterData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises UtmconverterAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise UtmconverterAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise UtmconverterAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise UtmconverterAPIClientError(f"Request failed: {str(e)}")
